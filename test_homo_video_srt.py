...
video_to_map_scale = np.array([7.,14.4])
    ...
    if r:
        ...
        if not kp1:
            ...
            if not counter:
                ...
                loc_delta = np.rint(res_d.mean(axis=0)/video_to_map_scale)                
                print(f"cur_loc {cur_loc} loc_delta {loc_delta} avg_delta {avg_delta}")     
                cur_loc = np.add(cur_loc, loc_delta.astype(int))
                ...
                if drow: 
                    ...
                    cv2.circle(img_map, cur_loc, 3, (0,0,255), -1)
