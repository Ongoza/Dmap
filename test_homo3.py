...
print("kp,des",type(kp1),type(des1))
if False:
...
if True:
    for i, match in enumerate(matches):
         ...
         print("m", i, mag, angl, x)     
         res_f[i][0] = mag
         res_f[i][1] = angl + 360 if angl < -180 else angl
avg_delta = res_f.mean(axis=0)
locs_delta = locs.mean(axis=0)
print(f"avg dist {avg_delta[0]} and angle {avg_delta[1]} locs{locs_delta}")     
