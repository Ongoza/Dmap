        ...
        if not kp1:
            ...
            if not counter:
                ...
                for i, match in enumerate(matches):
                    ...
                    res_xy[i] = np.absolute(np.sqrt(delta_1.dot(delta_1)) - np.sqrt(delta_2.dot(delta_2)))  # norm distance from center
                ...
                delta_loc = np.array([delta*np.cos(np.radians(cur_az)),delta*np.sin(np.radians(cur_az))])# if xy_delta > 0.02 else 0
                ...
                gps_new = wgs84_geod.fwd(gps_out[-1][0], gps_out[-1][1], cur_az, delta*scale_img2gps)
