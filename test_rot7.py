...
out_scale = 2
drone_delta = 32 # dif for check nearest locations
drone_delta_rot = 36 # dif for check nearest locations for rotation
...
def cam_virt_map(loc):    
    ...
    for i in range(-1,2,1):
        for j in range(-1,2,1):
            l = [loc[0]+i*drone_delta, loc[1]+j*drone_delta]
            print("ij", i,j,l)
            cv2.circle(img1, l, 5, (255, 255, 255), -1)
            #tr_0, loc_temp = rotate(l, 0, rec_temp, limit)         
            #tr, img = cam_virt_shot(l, 0, arrow, draw)
            if tr:
                out_locs.append(l)
                #out_imgs.append(img)
    return out_locs, out_imgs
...
for i in range(3,5,1):
    ...
    img2, locs = cam_virt(route[i], route[i+1], True)
    imgs = cam_virt_map(route[i])
    #cv2.circle(img1, route[i], 10, (0,200,0), -1)
    route_imgs += img2
    route_locs += locs
    #print("len img2",len(img2), np.array(route_imgs).shape, len(route_locs), type(route_locs), route_locs[0])
